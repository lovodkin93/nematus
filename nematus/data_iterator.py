...
class TextIterator:
    ...
    def __init__(self, source, target,
        ...
        if keep_data_in_memory:
            ...
            if source_same_scene_mask:
                self.source_same_scene_mask = FileWrapper(source_same_scene_mask, lines_file_path)
            else:
                self.source_same_scene_mask = None
            if source_parent_scaled_mask:
                self.source_parent_scaled_mask = FileWrapper(source_parent_scaled_mask, lines_file_path)
            else:
                self.source_parent_scaled_mask = None
            if source_UD_distance_scaled_mask:
                self.source_UD_distance_scaled_mask = FileWrapper(source_UD_distance_scaled_mask, lines_file_path)
            else:
                self.source_UD_distance_scaled_mask = None

            if target_same_scene_mask:
                self.target_same_scene_mask = FileWrapper(target_same_scene_mask, lines_file_path)
            else:
            ...
            self.source, self.target, self.source_same_scene_mask, self.source_parent_scaled_mask, self.source_UD_distance_scaled_mask, self.target_same_scene_mask = shuffle.jointly_shuffle_files(
                    [self.source_orig, self.target_orig, self.source_same_scene_mask_orig, self.source_parent_scaled_mask_orig,self.source_UD_distance_scaled_mask_orig, self.target_same_scene_mask_orig], self.lines_file_path, temporary=True)
    ...
    def reset(self):
        if self.preprocess_script:
            ...
            if self.keep_data_in_memory:
                self.source, self.target = FileWrapper(self.source_orig, self.lines_file_path), FileWrapper(self.target_orig, self.lines_file_path)
                self.source_same_scene_mask = FileWrapper(self.source_same_scene_mask_orig, self.lines_file_path) if self.source_same_scene_mask_orig else None
                self.source_parent_scaled_mask = FileWrapper(self.source_parent_scaled_mask_orig, self.lines_file_path) if self.source_parent_scaled_mask_orig else None
                self.source_UD_distance_scaled_mask = FileWrapper(self.source_UD_distance_scaled_mask_orig, self.lines_file_path) if self.source_UD_distance_scaled_mask_orig else None
                self.target_same_scene_mask = FileWrapper(self.target_same_scene_mask_orig, self.lines_file_path) if self.target_same_scene_mask_orig else None
            else:
    ...
    def __next__(self):
        ...
        # if not self.source_same_scene_mask_orig: # make sure same_scene_mask is of same length as source (so iterator will work) even when not used
