...
class AttentionBlock(object):
    ...
    def forward(self, inputs, memory_context, attn_mask, pre_softmax_scaled_attn_mask,post_softmax_scaled_attn_mask, cross_attention_keys_update_rules=None, layer_memories=None, isDecoder=False): # make sure everyone calling sends pre_softmax_scaled_attn_mask
        ...
        attn_outputs, layer_memories, attn_softmax_weights = self.attn.forward(attn_inputs, memory_context, attn_mask, pre_softmax_scaled_attn_mask,post_softmax_scaled_attn_mask, layer_memories, cross_attention_keys_update_rules=cross_attention_keys_update_rules, isDecoder=isDecoder) #goes to MultiHeadAttentionLayer's forward in nematus.transformer_attention_modules
